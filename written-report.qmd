...
figure1 <- ggplot(data = airbnb, aes(x = price)) +
...
figure2 <- ggplot(airbnb, aes(x = price)) +
  geom_histogram(bins = 30) +
  labs(title = "Closer Look at Distribution of Price 
       (Removed Outliers)", 
       x = "Listing Price (USD)", 
       y = "Count") +
  xlim(0, 2500) 
...
figure3 <- ggplot(data = airbnb, aes(x = bedrooms)) +
...
figure4 <- ggplot(data = airbnb, aes(x = review_scores_rating)) +
  geom_histogram() +
...
```{r}
#| label: figure-08
figure5 <- ggplot(airbnb, aes(x = room_type)) +
...
figure6 <- ggplot(airbnb, aes(x = neighbourhood_group_cleansed)) +
  geom_bar(fill = "lightblue", color = "black") +
  labs(
